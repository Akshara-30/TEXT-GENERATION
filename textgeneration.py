  import numpy
	...
	file=open("frankestein_2.txt").read()
	def tokenize_words(input):
	    input =input.lower()
	    tokenizer=RegexpTokenizer(r\'w+')
	    tokens=tokenizer.tokensize(input)
	...
	vocab_len=len(chars)
	print("total numbers of character:",input_len)
	print("total vocab:",vocab_len)
	seq_length=100
	x_data=100
	y_data=100
	for i in range(0,input_len_seq_length,1):
  ...
  model.add(Dropout(0.2))
	...
	filepath='model_weight_saved.hdfs'
	checkpoint=modelcheckpoint(filepath,monitor="loss",verbose=1,save_best_only=True,mode='min')
	desired_callbacks=(checkpoint)
	model.fit(x,y,epoch=4,batch_size=256,callbacks=desired_callbacks)
  ...
  print("Random Seed:")
	...
	for i in range(1000):
	      ...
	      seq_in=[numto_char[value]for value in pattern]
